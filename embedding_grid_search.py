...
for r in trange(len(5), desc='Rounds', leave=True):
    target_scores = {}
    for target in tqdm(round_targets[r+1], desc='Columns', leave=False):
        ...
        for i, closest_train_idx in enumerate(closest_indices):
            y_pred.append(labels['naics_code'].iloc[closest_train_idx])
            y_true.append(labels['naics_code'].iloc[test_idx[i]])
...
with open(f'{model_name}_scores.json') as file:
    json.dump(file)
